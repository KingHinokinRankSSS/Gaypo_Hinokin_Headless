    ...
    const run = function (x, config, oa) {
      ...
      trigger = {
        ...
        keydown: function (code, repeat) {
          const keyMap = { Enter: 'Enter', Escape: 'Escape', Space: ' ', Backspace: 'Backspace' };
          const key = keyMap[code] || (code && code.startsWith('Key') ? code.slice(3) : code);
          const keyCode = code === 'Enter' ? 13 : code === 'Escape' ? 27 : code === 'Space' ? 32 : 0;
      ...
      let inputs = [], setFocusedValue = function (input, str) {
        if (!input) return;
        input.value = str;
        const listeners = elementListeners.get(input);
        if (listeners) {
          const event = { target: input, isTrusted: true, inputType: 'insertText', data: str };
          if (listeners.input) {
      ...
      global.document = global.window.document = (function () {
        ...
        const createElement = function (tag, options) {
          const element = {
            ...
            focus: function () { activeElement = element; },
            blur: function () { if (activeElement === element) activeElement = null; },
            remove: emptyFunc,
      ...
      let i = 0, controller = {
        ...
        chat: function (str) {
          ...
          // Open/focus the game's chat input.
          controller.press('Enter');
          global.performance.time += 100;
          if (typeof a === 'function') a();

          // Prefer the actual focused input so the bot's player-name field or
          // other inputs are not overwritten by chat traffic. Fall back to the
          // legacy all-input setter only when the page did not expose focus.
          const focused = global.document && global.document.activeElement;
          if (focused && (focused.tag === 'input' || focused.tag === 'textarea')) {
            setFocusedValue(focused, message);
          } else {
            setValue(message);
          }
